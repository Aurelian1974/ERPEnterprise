...
export const localityKeys = {
  ...
  countries: ["localities", "countries"] as const,
  counties: (countryCode?: string | null): readonly ["localities", "counties", string] =>
    ["localities", "counties", countryCode ?? ""] as const,
  search: (
    q: string,
    county?: string | null,
    countryCode?: string | null,
  ): readonly ["localities", "search", string, string, string] =>
    ["localities", "search", q, county ?? "", countryCode ?? ""] as const,
};

export function useCountries() {
  return useQuery({
    queryKey: localityKeys.countries,
    queryFn: async () => {
      const { data } = await apiClient.get<CountryDto[]>(`${BASE}/localities/countries`);
      return data;
    },
    staleTime: Infinity,
  });
...
export function useCounties(countryCode?: string | null) {
  return useQuery({
    queryKey: localityKeys.counties(countryCode),
    queryFn: async () => {
      const params: Record<string, string> = {};
      if (countryCode) params.countryCode = countryCode;
      const { data } = await apiClient.get<CountyDto[]>(`${BASE}/localities/counties`, { params });
      return data;
    },
    enabled: !!countryCode,
    staleTime: Infinity,
...
export function useSearchLocalities(q: string | undefined, county?: string | null, countryCode?: string | null) {
  const query = q ?? "";
  const params: Record<string, string | number> = { q: query, limit: 10 };
  if (county) params.county = county;
  if (countryCode) params.countryCode = countryCode;

  const canSearch = query.trim().length >= 2 && !!countryCode;

  return useQuery({
    queryKey: localityKeys.search(query, county, countryCode),
    queryFn: async () => {
      const { data } = await apiClient.get<LocalityDto[]>(`${BASE}/localities/search`, { params });
      return data;
    },
    enabled: canSearch,
    staleTime: 60_000,
...
export function useValidateLocality() {
  return useMutation({
    mutationFn: async ({
      name,
      county,
      ...
      if (county) params.county = county;
      if (countryCode) params.countryCode = countryCode;
      const { data } = await apiClient.get<LocalityValidationDto>(`${BASE}/localities/validate`, { params });
      return data;
    },
  });
}
